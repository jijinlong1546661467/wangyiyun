    ...
    <div>
        ...
        <div class="pl-[4.2vw] pr-[4.2vw]">
            ...
            <div class="flex items-center justify-between h-[12vw] mb-[2vw]" style="border-bottom: 1px solid #f5f5f5">
                ...
                <van-popup v-model="nickNameBoolean" position="bottom" :style="{ height: '100%' }">
                    <div class="flex text-[4.4vw] items-center pl-[1vw] pt-[4.2vw] mb-[3.8vw] w-[100%] pr-[1vw]"
                        style="justify-content: space-between; align-items: center;">
                        <div class="flex" style="justify-content: space-between; align-items: center;">
                            <Icon icon="ri:arrow-left-line" style="font-size: 7vw" class="font-600 text-[black] w-[10vw]"
                                @click.native="nickNamePopuptf()" />
                            <p class="font-[600]">修改昵称</p>
                        </div>
                        <span class="font-[600]" @click="NMconfirmation()">保存</span>
                    </div>
                    <input type="text" v-model="userName" ref="inputField"
                        class="w-[100%] h-[12vw] px-[4vw] font-[600] text-[#333334] mb-[3vw] " @input="userNameDetection"
                        placeholder="请输入昵称" style="text-indent: ;font-size:4vw; border-bottom: 1px solid gray;" />
                    <div class="px-[4vw] flex items-center text-[#eb4d44] text-[2.6vw]"
                        :style="{ display: !promptText ? 'none' : 'flex' }">
                        <Icon :icon="promptText === '昵称可用～' ? 'icon-park-twotone:correct' : 'dashicons:warning'"></Icon>
                        {{ promptText }}
                    </div>
                    <span class="px-[3vw] py-[2.5vw] bg-[#f1f1f1] mx-[4vw] mt-[3vw] rounded-[2vw] text-[5vw]"
            ...
            <div class="flex items-center justify-between h-[12vw] mb-[2vw]" style="border-bottom: 1px solid #f5f5f5">
                ...
                <van-popup v-model="genderBoolean" position="bottom" style="height: 50vh;">
                    <van-picker show-toolbar :columns="genderList" :default-index="gender" @confirm="genderConfirm"
                        @cancel="gendercancel" @confirm-button-text="完成" @cancel-button-text="取消" />
                </van-popup>
            </div>
            ...
            <div class="flex items-center justify-between h-[12vw] mb-[2vw]" style="border-bottom: 1px solid #f5f5f5">
                ...
                <van-popup v-model="showsr">
                    <van-datetime-picker v-model="currentDate" type="date" title="选择年月日" :min-date="minDate"
                        :max-date="maxDate" @cancel="showxxsr" @confirm="confirmsr" />
                </van-popup>
            </div>
            <div class="flex items-center justify-between h-[12vw] mb-[2vw]" style="border-bottom: 1px solid #f5f5f5">
                <p>地区</p>
                ...
                <van-popup v-model="show">
                    ...
                    <van-area :area-list="areaList" :columns-num="2" @confirm="confirm" @cancel="showxx" />
                </van-popup>
            </div>
            <div class="flex items-center justify-between h-[12vw] mb-[2vw]" style="border-bottom: 1px solid #f5f5f5">
                <p>大学</p>
